function   [Delta_Mu, Delta_C, Delta_w, WinSet] = Grad_MuCW(VGmethod, trueModel, inferModel, PhiType, PsiType, BPtype, fix, opt, par, WinSet)
...
for jj = 1:NumberZ
    ...
    while Flag_outlier == 1
        ...
        BPcdf_basiseval = BPcdf_basis(u, k, BPtype); % Step 3
        tmpB = sum(BPcdf_basiseval.*W_old, 2); % Step 5
        tmpx = iPsi_f(tmpB, PsiPar, PsiType); % Step 8
