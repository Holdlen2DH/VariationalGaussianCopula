...
for iter = 1: MaxIter
    ...
    for inner = 1:InnerIter
        ...
        [Delta_Mu, Delta_C, Delta_w, WinSet] = Grad_MuCW(VGmethod, trueModel, inferModel, PhiType, PsiType, BPtype, fix, opt, par,WinSet);
        if opt.normalize ==1
        ...
        if opt.updateMuC~=1
            Delta_C = zeros(size(Delta_C));
            Delta_Mu = zeros(size(Delta_Mu));
        end
        %% Update Mu
        ...
        if strcmp(trueModel, 'LogNormal2')==1
            RMSE(inner, iter) = norm(corrcov(par.C *par.C')- fix.trueUpsilon, 'fro')./norm(fix.trueUpsilon, 'fro');
        end
    end
