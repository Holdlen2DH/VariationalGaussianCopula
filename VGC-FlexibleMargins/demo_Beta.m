...
PsiType = 'Beta';  opt.PsiPar(1) = 2; opt.PsiPar(2) = 2;
...
%%  Plot
figplot.nb = 100;
figplot.Seqx = linspace(0, 1, figplot.nb)'; fix2.c = 1;
figplot.log_pM =  logmodel(figplot.Seqx, trueModel, fix);
trueModel2 = PsiType;
fix2.alpha = opt.PsiPar(1) ;  fix2.beta = opt.PsiPar(2) ;  fix2.c = 1;
figplot.log_pM2 =  logmodel(figplot.Seqx, trueModel2, fix2);

figure
figplot.ls =1; figplot.ms =1;
set(gca,'fontsize', 14)
plot(figplot.Seqx, exp(figplot.log_pM), '-', 'linewidth', 3.5*figplot.ls, 'Markersize', 4*figplot.ms, 'Color', 'k');
hold on
plot(figplot.x_x1,figplot.f_x1/trapz(figplot.x_x1,figplot.f_x1),  '-o', 'linewidth', 2*figplot.ls, 'Markersize', 5*figplot.ms, 'Color', 'g');
hold on
plot(figplot.x_x2,figplot.f_x2/trapz(figplot.x_x2,figplot.f_x2), '--', 'linewidth', 3*figplot.ls, 'Markersize', 4*figplot.ms, 'Color', 'b');
hold on
plot(figplot.Seqx, exp(figplot.log_pM2)./trapz(figplot.Seqx, exp(figplot.log_pM2)), '-*', 'linewidth', 2*figplot.ls, 'Markersize', 4.5*figplot.ms, 'Color', 'r');
hold off
legend('Ground Truth', 'VIT-BP(k=5)', 'VGC-BP(k=5)' , 'Predefined \Psi')
